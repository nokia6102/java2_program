...
class J1105_FrameWindow extends Frame	//建立框架視窗類別
	implements MouseListener, MouseMotionListener	//執行滑鼠監聽介面
{
	...
	J1105_FrameWindow(String title)	//框架視窗建立者
	{
		super(title);	//定義視窗標題
	...
	public void paint(Graphics g)	//處理視窗繪圖
	{
		if(mouseUp) {	//若滑鼠放開旗號為真
			g.drawLine(start.x, start.y, stop.x, stop.y);	//畫直線
			for(int i=0; i<count; i++)	//重劃前面的圖形迴圈
				g.drawLine(begin[i].x, begin[i].y,
					   end[i].x, end[i].y);	//重劃直線
		}
	}
...
public class J1105_DrawLine	//建立視窗應用程式類別
{
	public static void main(String[] args)	//main方法
	{
		J1105_FrameWindow fw = new J1105_FrameWindow("畫線程式");	//建立視窗物件
