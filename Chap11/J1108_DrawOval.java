...
class J1108_FrameWindow extends Frame	//建立框架視窗類別
	implements MouseListener, MouseMotionListener	//執行滑鼠監聽介面
{
	...
	J1108_FrameWindow(String title)	//框架視窗建立者
	{
		super(title);	//定義視窗標題
	...
	public void paint(Graphics g)	//處理視窗繪圖
	{
		if(mouseUp) {	//若滑鼠放開旗號為真
			g.drawOval(start.x, start.y, 
				  (stop.x-start.x), 
				  (stop.y-start.y));	//畫橢圓
			for(int i=0; i<count; i++)	//重劃前面的圖形迴圈
				g.drawOval(begin[i].x, begin[i].y,
					  (end[i].x-begin[i].x), 
					  (end[i].y-begin[i].y));	//重劃橢圓
		}
	}
...
public class J1108_DrawOval	//建立視窗應用程式類別
{
	public static void main(String[] args)	//main方法
	{
		J1108_FrameWindow fw = new J1108_FrameWindow("畫橢圓程式");	//建立視窗物件
